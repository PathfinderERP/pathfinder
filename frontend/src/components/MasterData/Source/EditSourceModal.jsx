...
const EditSourceModal = ({ source, onClose, onSuccess, isDarkMode }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL}/source/${source._id}`, {
                method: "PUT",
                headers: {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all">
            <div className={`rounded-2xl w-full max-w-md border shadow-2xl transition-all ${isDarkMode ? 'bg-[#1a1f24] border-gray-700' : 'bg-white border-gray-200'}`}>
                <div className={`flex justify-between items-center p-6 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
                    <h3 className={`text-2xl font-black uppercase tracking-tighter italic ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Edit <span className="text-cyan-500">Source</span></h3>
