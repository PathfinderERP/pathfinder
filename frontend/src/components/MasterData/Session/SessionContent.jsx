...
const SessionContent = () => {
    ...
    const [formData, setFormData] = useState({ sessionName: "" });
    ...
    useEffect(() => {
        ...
        if (storedUser) {
            ...
            setCanCreate(hasPermission(parsedUser, 'masterData', 'session', 'create'));
            setCanEdit(hasPermission(parsedUser, 'masterData', 'session', 'edit'));
            setCanDelete(hasPermission(parsedUser, 'masterData', 'session', 'delete'));
        }
        fetchSessions();
    }, []);
    ...
    const openModal = (session = null) => {
        if (session) {
            ...
            setFormData({ sessionName: session.sessionName });
        } else {
            setCurrentSession(null);
            setFormData({ sessionName: "" });
        }
        setIsModalOpen(true);
    };
    ...
    const handleSave = async (e) => {
        ...
        const url = currentSession
            ? `${import.meta.env.VITE_API_URL}/session/update/${currentSession._id}`
    ...
    const handleDelete = async (id) => {
        ...
        const token = localStorage.getItem("token");
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/session/delete/${id}`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Bearer ${token}`
                }
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-800">
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan="3" className="p-8 text-center text-gray-500">No sessions found</td>
                            </tr>
                        ) : (
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
                    <div className="bg-[#1a1f24] p-6 rounded-lg w-full max-w-md border border-gray-700 shadow-xl">
                        ...
                        <form onSubmit={handleSave}>
                            <div className="mb-4">
                                <label className="block text-gray-400 mb-2 text-sm">Session Name</label>
                                <input
                                    type="text"
                                    name="sessionName"
