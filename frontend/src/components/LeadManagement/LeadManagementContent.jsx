    ...
    const fetchFilterData = useCallback(async () => {
        try {
            ...
            if (userResponse.ok) {
                ...
                const isManagerial = ['superadmin', 'super admin', 'admin', 'centerincharge', 'centreincharge', 'zonalmanager', 'hod'].includes(currentUser.role?.toLowerCase()?.replace(/\s+/g, ''));
                ...
                <div className={`border rounded-[2px] p-8 space-y-6 ${isDarkMode ? 'bg-[#131619] border-gray-800' : 'bg-white border-gray-200 shadow-sm'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-9 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <CustomMultiSelect
                                options={
                                    ['superadmin', 'super admin', 'admin', 'centerincharge', 'centreincharge', 'zonalmanager', 'hod'].includes(user?.role?.toLowerCase()?.replace(/\s+/g, ''))
                                ...
                                isDisabled={!['superadmin', 'super admin', 'admin', 'centerincharge', 'centreincharge', 'zonalmanager', 'hod'].includes(user?.role?.toLowerCase()?.replace(/\s+/g, ''))}
                                theme={isDarkMode ? 'dark' : 'light'}
                            />
                        </div>
                        <div className="space-y-2">
