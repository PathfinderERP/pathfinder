...
import { FaArrowLeft, FaPrint, FaDownload, FaEnvelope, FaFileAlt, FaSpinner, FaHistory, FaBriefcase, FaExternalLinkAlt, FaTrash, FaSignature, FaTimes, FaPlus, FaSave, FaMagic } from "react-icons/fa";
...
const AppointmentLetter = () => {
    ...
    const fetchTemplates = React.useCallback(async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`${import.meta.env.VITE_API_URL}/hr/letters/templates/Appointment Letter`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (response.ok) {
                const data = await response.json();
