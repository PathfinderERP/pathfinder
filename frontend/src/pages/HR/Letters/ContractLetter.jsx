...
const ContractLetter = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`${import.meta.env.VITE_API_URL}/hr/letters/contract/${id}`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`
            ...
            if (response.ok) {
                ...
                toast.success("Employment contract generated successfully!");
                fetchEmployeeDetails();
            } else {
    ...
    const handleSendEmail = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`${import.meta.env.VITE_API_URL}/hr/letters/contract/${id}/send`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`
    ...
    const history = employee?.letters?.filter(l => l.letterType === "Contract Letter") || [];
            ...
            <div className="space-y-6 animate-fade-in pb-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Contract Letter</h1>
