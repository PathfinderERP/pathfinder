    ...
    return (
        <Layout activePage="Employee Center">
            <div className="p-6 space-y-6 animate-fade-in">
                <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl border ${isDarkMode ? 'bg-[#1a1f24] border-gray-800' : 'bg-white border-gray-100 shadow-sm'}`}>
                    <div>
                        <h1 className={`text-2xl font-bold ${isDarkMode?'text-white':'text-gray-800'}`}>Team Regularization</h1>
                        <p className={`text-sm ${isDarkMode?'text-gray-400':'text-gray-500'}`}>Approve or edit attendance requests for your reporting employees</p>
                    </div>
                </div>

                {/* Filters */}
                <div className={`flex flex-wrap gap-4 p-4 rounded-xl border items-center ${isDarkMode?'bg-[#1a1f24] border-gray-800':'bg-white border-gray-100 shadow-sm'}`}>
                    <input 
                        type="date" 
                        value={localFilters.date} 
                        onChange={(e) => setLocalFilters({ ...localFilters, date: e.target.value })}
                        className={`px-4 py-2 border rounded-lg text-sm outline-none ${isDarkMode?'bg-gray-800 border-gray-700 text-white':'bg-gray-50 border-gray-200 text-gray-800'}`}
                        title="Filter by Date"
                    />
                    <select
                        value={localFilters.center}
                        onChange={(e) => setLocalFilters({ ...localFilters, center: e.target.value })}
                        className={`px-4 py-2 border rounded-lg text-sm outline-none min-w-[150px] ${isDarkMode?'bg-gray-800 border-gray-700 text-white':'bg-gray-50 border-gray-200 text-gray-800'}`}
                    >
                    ...
                    <input 
                        ...
                        className={`px-4 py-2 border rounded-lg text-sm outline-none min-w-[150px] flex-1 max-w-[200px] ${isDarkMode?'bg-gray-800 border-gray-700 text-white':'bg-gray-50 border-gray-200 text-gray-800'}`}
                    />
                    <input 
                        type="text" 
                    ...
                    <select
                        ...
                        className={`px-4 py-2 border rounded-lg text-sm outline-none w-full sm:w-auto ${isDarkMode?'bg-gray-800 border-gray-700 text-white':'bg-gray-50 border-gray-200 text-gray-800'}`}
                    >
                            ...
                            <tbody className={`divide-y ${isDarkMode?'divide-gray-800':'divide-gray-100'}`}>
                                {loading ? (
                                    ...
                                    requests.filter(request => {
                                        ...
                                        <tr key={request._id} className={`transition-colors ${isDarkMode?'hover:bg-white/5':'hover:bg-gray-50'}`}>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${
                                                    request.type === 'On Duty' ? (isDarkMode?'bg-purple-500/10 text-purple-400':'bg-purple-100 text-purple-600') :
                        ...
                        <form onSubmit={handleStatusUpdate} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 animate-fade-in">
                                <div className="space-y-1.5">
                                    ...
                                    <input type="time" value={reviewData.fromTime} onChange={(e) => setReviewData({ ...reviewData, fromTime: e.target.value })} className={`w-full px-4 py-2 border rounded-xl outline-none text-sm ${isDarkMode?'bg-gray-800 border-gray-700 text-white':'bg-gray-50 border-gray-200 text-gray-800'}`} />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold uppercase text-gray-500 ml-1">To Time</label>
                                    <input type="time" value={reviewData.toTime} onChange={(e) => setReviewData({ ...reviewData, toTime: e.target.value })} className={`w-full px-4 py-2 border rounded-xl outline-none text-sm ${isDarkMode?'bg-gray-800 border-gray-700 text-white':'bg-gray-50 border-gray-200 text-gray-800'}`} />
                                </div>
                            </div>

                            <div className="space-y-1.5 pt-2">
                                <label className="text-[10px] font-bold uppercase text-gray-500 ml-1">Manager Remark</label>
                                <textarea rows="3" value={reviewData.remark} onChange={(e) => setReviewData({ ...reviewData, remark: e.target.value })} className={`w-full px-4 py-3 border rounded-xl outline-none text-sm resize-none ${isDarkMode?'bg-gray-800 border-gray-700 text-white':'bg-gray-50 border-gray-200 text-gray-800'}`} placeholder="Reason for approval/rejection..." />
                            </div>
                            <div className="pt-2 flex gap-3">
                                <button type="button" onClick={() => setShowStatusModal(false)} className={`flex-1 px-4 py-3 rounded-xl font-bold border ${isDarkMode?'bg-gray-800 border-gray-700 text-gray-300':'bg-gray-100 border-gray-200 text-gray-600'}`}>Cancel</button>
