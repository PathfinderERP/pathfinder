                        ...
                        <section className={`rounded-3xl p-8 shadow-lg relative overflow-hidden ${isDarkMode ? 'bg-gradient-to-br from-gray-900 to-gray-800 text-white' : 'bg-gray-50 border border-gray-200 text-gray-900'}`}>
                            ...
                            {employee.salaryStructure && employee.salaryStructure.length > 0 ? (
                                (() => {
                                    ...
                                    let pf = latest.pf;
                                    let pfEmployer = latest.pfEmployer;
                                    let esi = latest.esi;
                                    let esiEmployer = latest.esiEmployer;
                                    ...
                                    if (!employee.isDeductions) {
                                        pf = 0; pfEmployer = 0; esi = 0; esiEmployer = 0; pTax = 0; tds = 0; lossOfPay = 0; adjustment = 0;
                                    }

                                    // Define Deductions - exactly matching HR view order and labels
                                    const deductions = [
                                        { label: "Provident Fund (PF)", val: pf },
                                        { label: "P.F. (Employer Contribution)", val: pfEmployer },
                                        { label: "ESI Contribution", val: esi },
                                        { label: "ESI (Employer Contribution)", val: esiEmployer },
