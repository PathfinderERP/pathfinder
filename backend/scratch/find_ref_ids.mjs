...
async function run() {
    try {
        ...
        console.log("Departments found:", departments.map(d => ({ _id: d._id, departmentName: d.departmentName })));

        // Look up all designations
        const designations = await db.collection('designations').find({}).toArray();
        console.log("Designations found:", designations.map(d => ({ _id: d._id, name: d.name })));
