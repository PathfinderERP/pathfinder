...
const bulkApproveExpenses = async (req, res) => {
    try {
        const { ids, givenBy, reason } = req.body;
        ...
        const expensesToApprove = await Expense.find({ _id: { $in: validIds } });
        let approvedCount = 0;

        for (const expense of expensesToApprove) {
            // Only approve if remaining amount > 0 or status is not fully Approved
            const originalAmount = expense.originalAmount !== undefined ? expense.originalAmount : (expense.amount || 0);
            const currentPaidAmount = expense.paidAmount || 0;
            const currentRemainingAmount = expense.remainingAmount !== undefined ? expense.remainingAmount : originalAmount;

            if (currentRemainingAmount <= 0 && expense.financeStatus === 'Approved') {
                continue; // Skip already fully approved
            }

            const actualAmountPaid = currentRemainingAmount; // Approve full remaining
            const newPayment = {
                amountPaid: actualAmountPaid,
                paidDate: new Date(),
                paidBy: req.user?._id || req.user?.id || null,
                givenBy: givenBy || "",
                reason: reason || ""
            };

            const updatedPayments = [...(expense.payments || []), newPayment];
            const newPaidAmount = currentPaidAmount + actualAmountPaid;
            ...
            await Expense.findByIdAndUpdate(expense._id, {
                ...
                financeApprovedBy: req.user?._id || req.user?.id || null,
                financeApprovedDate: new Date(),
                givenBy: givenBy || expense.givenBy || "",
        ...
        res.status(200).json({
            success: true,
            message: `Successfully approved ${approvedCount} expense(s)`,
            count: approvedCount
        });
    } catch (error) {
...
export { createExpense, getAllExpence, getSingleExpence, updateExpence, bulkImportExpenses, deleteExpense, bulkDeleteExpenses, bulkEditExpenses, bulkApproveExpenses };