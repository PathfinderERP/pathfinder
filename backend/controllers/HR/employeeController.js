    ...
    try {
        ...
        for (const data of employeesData) {
            try {
                ...
                // If employeeId is not provided, it will be auto-generated by pre-save hook
                // But we manually generate it here to use for User password if needed
                if (!data.employeeId) {
                    const now = new Date();
                    const year = now.getFullYear().toString().slice(-2);
                    const prefix = `EMP${year}`;
                    ...
                    if (lastRec && lastRec.employeeId) {
                        const lastSeq = parseInt(lastRec.employeeId.slice(5), 10);
                        if (!isNaN(lastSeq)) seq = lastSeq + 1;
                    }
                    data.employeeId = `${prefix}${String(seq).padStart(6, "0")}`;
                }
