...
export const getDiscountReport = async (req, res) => {
    try {
        ...
        if (centreIds) {
            ...
            let defaultNames = [];
            if (req.user.role !== 'superAdmin') {
                defaultNames = allowedCentreNames.filter(name => name && !/phsps/i.test(name) && !/franchise/i.test(name));
            } else {
                const activeCentres = await Centre.find({ status: { $ne: "deactive" } }).select("centreName");
                defaultNames = activeCentres.map(c => c.centreName).filter(name => name && !/phsps/i.test(name) && !/franchise/i.test(name));
            }
